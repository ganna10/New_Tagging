#!/usr/bin/env perl
#write out .eqn, .spc and mecca.f90 module from tagged MOZART mechanism using only inorganic chemistry
#Version 0: Jane Coates 7/5/2015

use strict;
use diagnostics;

my @RO2_species = qw( HO2 HO2_X_INI HO2_X_XTR );

my $species_file = "mozart.spc.inorganic";
open my $in, '<:encoding(utf-8)', $species_file or die $!;
my @lines = <$in>;
close $in;
my @species;
...
#add deposition
open my $dep_in, "<:encoding(utf-8)", "MOZART_deposition_velocities.txt.inorganic" or die $!;
...
my @primary_species = qw( NO NO_AIR );
...
#output files
my $eqn_file = "mozart.eqn.inorganic";
my $mecca_file = "mecca_mozart.f90.inorganic";

#Write eqn file
open my $eqn_out, '>:encoding(utf-8)', $eqn_file or die $!;
print $eqn_out "// inorganic\n";
